        ...
        <div style={{display: 'flex', alignItems: 'center', flexDirection: 'column', justifyContent: 'space-between'}}>
          ...
          { this.props.showIcons
            ? <div style={{ display: 'flex', alignItems: 'center', flexDirection: 'column', justifyContent: 'space-between', marginTop: 10 }}>
              { this.props.highlightIcon.toLowerCase() === 'initiate' ? <img style={{ paddingBottom: 5, cursor: 'pointer' }} src={initiatorcolor} alt='initiator' onClick={() => this.props.redirect('/initiator')} /> : <img style={{paddingBottom: 5, cursor: 'pointer'}} src={initiatorgray} alt='initiator' onClick={() => this.props.redirect('/initiator')} /> }
              { this.props.highlightIcon.toLowerCase() === 'find' ? <img style={{ paddingTop: 5, paddingBottom: 5, cursor: 'pointer' }} src={findercolor} alt='finder' onClick={() => this.props.redirect('/finder')} /> : <img style={{paddingTop: 5, paddingBottom: 5, cursor: 'pointer'}} src={findergray} alt='finder' onClick={() => this.props.redirect('/finder')} /> }
              { this.props.highlightIcon.toLowerCase() === 'plan' ? <img style={{ paddingTop: 5, paddingBottom: 5, cursor: 'pointer', height: 52 }} src={plannercolor} alt='planner' onClick={() => this.props.redirect('/planner')} /> : <img style={{paddingTop: 5, paddingBottom: 5, cursor: 'pointer'}} src={plannergray} alt='planner' onClick={() => this.props.redirect('/planner')} /> }
              { this.props.highlightIcon.toLowerCase() === 'do' ? <img style={{ paddingTop: 5, paddingBottom: 5, cursor: 'pointer', height: 52 }} src={doercolor} alt='doer' onClick={() => this.props.redirect('/doer')} /> : <img style={{paddingTop: 5, paddingBottom: 5, cursor: 'pointer'}} src={doergray} alt='doer' onClick={() => this.props.redirect('/doer')} /> }
              { this.props.highlightIcon.toLowerCase() === 'validate' ? <img style={{ paddingTop: 5, paddingBottom: 5, cursor: 'pointer', height: 52 }} src={validatorcolor} alt='validator' onClick={() => this.props.redirect('/validator')} /> : <img style={{paddingTop: 5, paddingBottom: 5, cursor: 'pointer'}} src={validatorgray} alt='validator' onClick={() => this.props.redirect('/validator')} /> }
              { this.props.highlightIcon.toLowerCase() === 'resolve' ? <img style={{ paddingTop: 5, cursor: 'pointer', height: 52 }} src={resolvercolor} alt='resolver' onClick={() => this.props.redirect('/resolver')} /> : <img style={{paddingTop: 5, cursor: 'pointer'}} src={resolvergray} alt='resolver' onClick={() => this.props.redirect('resolver')} /> }
            </div>
            : null
          }
        </div>
